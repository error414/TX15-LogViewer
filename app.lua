...
local function drawGraph_graph_lines_single_line(varIndex, points, min, max)
    ...
    local yScale = (max - min) / graphConfig.scale_ratio
    ...
    for i = 0, #points - 1, 1 do
        ...
        if min == max then
            y = graphConfig.DEFAULT_CENTER_Y + 6 * varIndex
            prevY = y
        end
...
local function drawGraph_bottom_session_line()
    ...
    lcd.drawLine(viewStart, LCD_H - 3, viewEnd, LCD_H - 3, SOLID, RED)
    lcd.drawLine(viewStart, LCD_H - 2, viewEnd, LCD_H - 2, SOLID, RED)
    lcd.drawLine(viewStart, LCD_H - 1, viewEnd, LCD_H - 1, SOLID, RED)
end
...
local function drawGraph_min_max()
    ...
    for varIndex = 1, 4, 1 do
        ...
        lcd.drawLine(x, LCD_H - 32, x, 250, SOLID, CUSTOM_COLOR)
...
local function drawGraph_cursor()
    ...
    for varIndex = 1, 4, 1 do
        ...
        -- cursor values
        local yScale = (varPoints.max - varPoints.min) / graphConfig.scale_ratio
